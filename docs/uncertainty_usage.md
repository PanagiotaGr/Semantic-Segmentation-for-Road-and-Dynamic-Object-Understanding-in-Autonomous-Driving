...
This guide shows how to use the uncertainty utilities in `src/research/uncertainty.py`.

## Motivation

Semantic segmentation models usually output one class per pixel. For autonomous driving, the model should also expose whether a prediction is reliable, especially near pedestrians, vehicles, road boundaries, occlusions, and unusual weather or lighting conditions.
...
Connect these utilities to an inference script so each predicted mask is saved together with:
